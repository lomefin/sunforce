...
//
// Character A "Caporal". Commanding high-step, heavy boots, jingling bells —
// rushdown with strong kicks. Boxes authored in CONCEPT SPACE, read straight
// off concept/caporal.svg (640x960, y-down). originX 302 = the crotch vertex of
// #legs; groundY 858 = the #ground-shadow ellipse cy. unitsPerPx 0.5 makes the
// sheet's 756px of character exactly 378 world units.
//
// M0 SCOPE: 5P and 5K ONLY. 2P, 2K, j.P, j.K and the P+K throw are deferred
// (docs/M0-VERTICAL-SLICE.md), and the throw's literal is kept verbatim in
// docs/MOVE-FORMAT.md so it drops back in as one paste plus two array entries:
// `moveOrder: [MoveId.A_THROW, MoveId.A_5K, MoveId.A_5P]` (throw FIRST, so P+K
// cannot be eaten by the punch matcher) and `moves: [A_5P, A_5K, A_THROW]`.
//
...
export const CHAR_A: CharDef = {
  ...
  moveOrder: [MoveId.A_5K, MoveId.A_5P],
  moves: [A_5P, A_5K],
  stick: {
    radii: [
      0, 13, 12, 14, 7, 17, 0,
